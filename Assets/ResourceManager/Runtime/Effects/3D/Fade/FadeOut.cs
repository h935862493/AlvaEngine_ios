...
public class FadeOut : EffectsBase
{
    //[Header("时间")]
    ...
    public void Execute()
    {
        if (!this.GetComponent<FadeOut>().enabled)
            return;
        if (IsExecute)
            return;
        IsExecute = true;
        ...
        for (int i = 0; i < materials.Count; i++)
        {
            ...
            Color color = m.color;
            m.color = new Color(color.r, color.g, color.b, fadetime);
    ...
    private void Update()
    {
        if (IsExecute)
        {
            ...
            for (int i = 0; i < materials.Count; i++)
            {
                ...
                m.color = new Color(color.r, color.g, color.b, 1 - total);
                //Debug.Log(m.color);
            }
            if (timeber > fadetime)
            {
...
*/
    ...
    public void Recover()
    {
        ...
        for (int i = 0; i < materials.Count; i++)
        {
            ...
            setMaterialRenderingMode(m, RenderingMode.Fade);
            //m.DOColor(new Color(color.r, color.g, color.b, 0), fadeTime);
            m.color = Color.Lerp(new Color(color.r, color.g, color.b, 1), new Color(color.r, color.g, color.b, 0), 0.1f);
            //Debug.Log(m.color);
        }
