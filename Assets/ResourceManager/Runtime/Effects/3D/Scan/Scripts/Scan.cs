...
[AddComponentMenu("myName/myProduct/v1/Linear Scan")]
public class Scan : MaterialEffectsBase
{
    ...
    {
        if (!this.GetComponent<Scan>().enabled)
            return;
        float min_z = 0.5f;
        float _edge = 0.08f;
        ...
        foreach (var renderer in renderers)
        {
            for (int i = 0; i < renderer.materials.Length; i++)
            {
                ...
                renderer.materials[i].SetInt("_AxisChoose", (int)axis);
    ...
    void Update()
    {
        if (IsExecute)
        {
            Treshold += Time.deltaTime * 0.5f / time;
