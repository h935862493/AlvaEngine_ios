...
typedef struct
{
    ...
    UnityARMatrix4x4 projectionMatrix;//投影矩阵
    ...
    UnityARMatrix4x4 viewMatrix;//相机视图矩阵
    UnityARMatrix4x4 modelMatrix;//模型矩阵
    int imageWidth;
    int imageHeight;
    int orientation;
    int isFoundModel;
...
inline void ARKitMatrixToFloatArray(const matrix_float4x4& matrixIn, float* matrixOut)
{
    vector_float4 c0 = matrixIn.columns[0];
    matrixOut[0]= c0.x;
    matrixOut[1] = c0.y;
...
inline void FloatArrayToARKitMatrix(const float * matrixIn,matrix_float4x4* matrixOut)
{
     
    matrixOut->columns[0]=simd_make_float4(matrixIn[0],matrixIn[1],matrixIn[2],matrixIn[3]);
    matrixOut->columns[1]=simd_make_float4(matrixIn[4],matrixIn[5],matrixIn[6],matrixIn[7]);
    matrixOut->columns[2]=simd_make_float4(matrixIn[8],matrixIn[9],matrixIn[10],matrixIn[11]);
    matrixOut->columns[3]=simd_make_float4(matrixIn[12],matrixIn[13],matrixIn[14],matrixIn[15]);
}
