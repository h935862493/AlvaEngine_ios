...
namespace UnityEngine.XR.iOS
{
    ...
    struct internal_UnityARCamera
    {
        ...
        public UnityARMatrix4x4 viewMatrix;//相机视图矩阵
        public UnityARMatrix4x4 modelMatrix;//模型矩阵
        public int imageWidth;
        public int imageHeight;
        public int orientation;
        public int isFoundModel;
    ...
    public struct UnityARCamera
    {
        ...
        public UnityARCamera(UnityARMatrix4x4 wt, UnityARMatrix4x4 pm, ARTrackingState ats, ARTrackingStateReason atsr, UnityVideoParams uvp, UnityARLightData lightDat, UnityARMatrix4x4 dt, ARPointCloud ptCloud, ARWorldMappingStatus awms, float focalx1,
         float focaly1,float principalx1,float principaly1, UnityARMatrix4x4 vm, UnityARMatrix4x4 mm,int iw,int ih,int ori,int isFound)
        {
            worldTransform = wt;
            projectionMatrix = pm;
            trackingState = ats;
            ...
            focalx = focalx1;
            focaly = focaly1;
            principalx = principalx1;
            principaly = principaly1;
            viewMatrix = vm;
            modelMatrix = mm;
            imageWidth = iw;
            imageHeight = ih;
            orientation = ori;
            isFoundModel = isFound;
        }
    }
...
#pragma warning restore CS0414
        ...
        {
            //Debug.Log("ARKitWorldTrackingSessionConfiguration");
            this.getPointCloudData = getPointCloudData;
            this.alignment = alignment;
    ...
    public partial class UnityARSessionNativeInterface
    {
        ...
        [DllImport("__Internal")]
		private static extern void setArType(int unityArType,int rotation);


        [DllImport("__Internal")]
		private static extern void unitMT();

        [DllImport("__Internal")]
		private static extern void unitIR();

        [DllImport("__Internal")]
		private static extern void setALVACoreInfo(string unityCompanyName,string unityAuthString,string unityModelPath,IntPtr guiderImage);

        [DllImport("__Internal")]
		private static extern bool isInitARSDK();

        [DllImport("__Internal")]
		private static extern int isInitSucceed();
        ...
        static void _frame_update(internal_UnityARCamera camera)
        {
            ...
            pubCamera.focalx = camera.focalx;
            pubCamera.focaly = camera.focaly;
            pubCamera.principalx = camera.principalx;
            pubCamera.principaly = camera.principaly;

            pubCamera.viewMatrix = camera.viewMatrix;
            pubCamera.modelMatrix = camera.modelMatrix;
            pubCamera.imageWidth = camera.imageWidth;
            pubCamera.imageHeight = camera.imageHeight;
            pubCamera.orientation = camera.orientation;
            pubCamera.isFoundModel = camera.isFoundModel;
        ...
        public void SetArType(int unityArType,int rotation)
        {
#if !UNITY_EDITOR && UNITY_IOS
             setArType(unityArType,rotation);
#endif
        }
        ...
        public void SetALVACoreInfo(string unityCompanyName, string unityAuthString, string unityModelPath,IntPtr guiderImage)
        {
#if !UNITY_EDITOR && UNITY_IOS
             setALVACoreInfo(unityCompanyName,unityAuthString,unityModelPath,guiderImage);
#endif
        }

      

        public bool IsInitARSDK()
        {
#if !UNITY_EDITOR && UNITY_IOS
           return  isInitARSDK();
#else
