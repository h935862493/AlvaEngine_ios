...
namespace UnityEngine.XR.iOS.Utils
{
	...
	public class serializableUnityARCamera
	{
		...
		public serializableUnityARCamera( serializableUnityARMatrix4x4 wt, serializableUnityARMatrix4x4 pm, ARTrackingState ats, ARTrackingStateReason atsr, UnityVideoParams uvp, UnityARLightData lightDat, serializableUnityARMatrix4x4 dt, serializablePointCloud spc, 
				ARWorldMappingStatus awms, float focalx1,float focaly1,float principalx1,float principaly1,UnityARMatrix4x4 vm,
		 UnityARMatrix4x4 mm,int iw,int ih,int ori,int isFound
		 )
		{
			worldTransform = wt;
			projectionMatrix = pm;
		...
		{
			return new UnityARCamera (rValue.worldTransform, rValue.projectionMatrix, rValue.trackingState, rValue.trackingReason, rValue.videoParams, rValue.lightData,
				rValue.displayTransform, rValue.pointCloud, rValue.worldMappingStatus, rValue.focalx, rValue.focaly, rValue.principalx,rValue.principaly,
				rValue.viewMatrix,rValue.modelMatrix,rValue.imageWidth,rValue.imageHeight,rValue.orientation,rValue.isFoundModel
				);
		}
#else //!UNITY_EDITOR
		public static implicit operator serializableUnityARCamera(UnityARCamera rValue)
		{
			return new serializableUnityARCamera(rValue.worldTransform, rValue.projectionMatrix, rValue.trackingState, rValue.trackingReason, rValue.videoParams, rValue.lightData, rValue.displayTransform, rValue.pointCloud, rValue.worldMappingStatus,rValue.focalx, rValue.focaly, rValue.principalx,rValue.principaly,
			rValue.viewMatrix,rValue.modelMatrix,rValue.imageWidth,rValue.imageHeight,rValue.orientation,rValue.isFoundModel);
		}
#endif
	};
